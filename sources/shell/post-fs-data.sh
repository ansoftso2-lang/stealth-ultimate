...
# v5.8h3: Only add KNOWN root-detection apps. The previous script added
# ANY package matching detect|root|magisk|checker — that swept in file
# managers, terminals, and legitimate apps, which then got hooked too.
# We only need actual detector apps on the denylist.
DENY_PKGS="com.drsniffer.detector io.github.vvb2060.magiskdetector com.reveny.nativecheck"
if [ -x /data/adb/magisk/magisk ]; then
